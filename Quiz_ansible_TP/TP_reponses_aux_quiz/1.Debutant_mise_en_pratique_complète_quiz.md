...
Voici ta mise en pratique complète mise en forme, moderne et avec des icônes Markdown pour une meilleure lisibilité :
...
📂 Crée le dossier `files/` et le fichier `files/index.html` :
...
- L’utilisateur `deploy` existe (`getent passwd deploy`)
- Le répertoire `/var/www/myapp` avec bons droits
- Le fichier `index.html` créé
...
✨ *But : socle plus “pro” : secrets, verification qualité, exécution fine*

1) Ajouter un secret chiffré (même symbolique):
...
⏯️ Relancer le playbook avec Vault :
...
2) Ajouter des **tags** dans les rôles :
...
⏯️ Exemple : lancer uniquement nginx :
...
4) **ansible-lint** (analyse la qualité) :
...
Corrige ce qu'il signale (permissions, modules, FQCN…).

---

## 🏆 Socle acquis

Tu as pratiqué sur un cas concret :

- Structure claire (ansible.cfg, inventories, playbooks, roles, group_vars)
- Modules clés (apt, service, user, file, copy, template)
- Idempotence et bonnes pratiques
- Architecture par rôles
- Handlers et templates dynamiques
- Vault pour secrets
- Tags, check-mode pour exécution fine
- ansible-lint pour qualité et propreté
...
Et dans quel template (ou tâche) l’utilises-tu ?
